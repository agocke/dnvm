...
using static Dnvm.RollForwardComparisons;
...
public static partial class RestoreCommand
{
    ...
    public static async Task<Result<SemVersion, Error>> Run(DnvmEnv env, Logger logger, Options options)
    {
        ...
        else
        {
            ...
            var error = await InstallCommand.InstallSdk(
                @lock,
                env,
                manifest,
                release.Sdk,
                release,
                manifest.CurrentSdkDir,
                InstalledSdk.RollForwardOptions.Disable, // restored SDKs don't roll forward
                logger
            );
